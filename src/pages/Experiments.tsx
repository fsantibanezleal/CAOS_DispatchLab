  ...
  return (
    <div className="page-body prose">
      ...
      <section>
        ...
        <p>{es ? 'Ocho casos cubren los ejes donde las políticas divergen: balance de flota (sub/equilibrado/sobre-camionado), número de palas (1→4), simetría de caminos, y los controles (1×1 oráculo con respuesta cerrada; balanceado donde greedy≈óptimo).' : 'Eight cases span the axes where policies diverge: fleet balance (under/balanced/over-trucked), number of shovels (1→4), road symmetry, and the controls (1×1 oracle with a closed-form answer; balanced where greedy≈optimal).'}</p>
        <table className="cmp-table">
          <thead><tr><th className="lo">{es ? 'Caso' : 'Case'}</th><th>{es ? 'Palas' : 'Shovels'}</th><th>{es ? 'Eje' : 'Axis'}</th></tr></thead>
          <tbody>
            {([['C01', 1, es ? 'balanceado (control)' : 'balanced (control)'], ['C02', 1, es ? 'sobre-camionado' : 'over-trucked'], ['C03', 1, es ? 'sub-camionado' : 'under-trucked'], ['C04', 2, es ? 'simétrico' : 'symmetric'], ['C05', 2, es ? 'asimétrico' : 'asymmetric'], ['C06', 3, es ? 'multi-pala' : 'multi-shovel'], ['C07', 4, es ? 'rico/asimétrico' : 'rich/asymmetric'], ['C12', 1, es ? 'oráculo 1×1' : '1×1 oracle']] as [string, number, string][]).map(([id, n, ax]) => (
