...
```
8 dispatch scenarios (CONTRACT 1) ──► Node DES dataset gen (science/gen_dataset.mjs, the SAME TS DES) ──► decisions (jsonl, git-ignored)
                                                                                                            │
                                                  ...
                                                  8 cases × policies (science/bake_cases.mjs, the TS DES) ───┴─► case-results.json               ┘ (committed)
                                                                                                            │
...
* **`data-pipeline/dlab/`** — the offline engine: `io/` (contracts, formats), `core/` (rng, trace, manifest, gate),
  `stages/` (the named pipeline, thin wrappers over the science), `cases/` + `registry.py` (the 8 cases by category),
...
* **`frontend/`** — the React/Vite SPA: `src/sim/` (the TS DES engine), `src/policies/` (the 5 heuristics + the
  learned policies), `src/lib/ort.ts` (onnxruntime-web), `src/viz/` (PitMap / Pareto / sweep), `src/pages/` (the 6
  standard pages), `src/lib/contract.types.ts` (the Contract-2 mirror).
