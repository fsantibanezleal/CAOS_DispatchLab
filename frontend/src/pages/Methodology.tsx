...
function Rollout({ es }: { es: boolean }) {
  return (<>
    ...
    <p>{es
      ? 'Sobre el modelo DETERMINISTA la cota de mejora se cumple exactamente: el rollout es >= su base en todo el corpus, con una ganancia REAL solo en el caso asimétrico C05 (~1%, +872 t) y empates en las flotas homogéneas y los controles (C01/C04/C12). Bajo ruido, el rollout de equivalencia-cierta NO supera a la asignación miope (la base ya está a pocos % del oráculo de capacidad, y la ventaja del look-ahead es frágil al ruido). Se publica el NULO honesto: el horizonte compra ~0 para flotas homogéneas, la cota de mejora determinista es la contribución validada. Ver Benchmark.'
      : 'On the DETERMINISTIC model the improvement bound holds exactly: the rollout is >= its base across the whole corpus, with a REAL gain only on the asymmetric C05 (~1%, +872 t) and ties on the homogeneous fleets and the controls (C01/C04/C12). Under noise, the certainty-equivalent rollout does NOT beat myopic assignment (the base is already within a few % of the capacity oracle, and the look-ahead edge is fragile to noise). The honest NULL is shipped: horizon buys ~0 for homogeneous fleets, the deterministic improvement bound is the validated contribution. See Benchmark.'}</p>
