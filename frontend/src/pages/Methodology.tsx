...
function Pol({ es }: { es: boolean }) {
  return (<>
    <p>{es ? 'Las políticas comparten una base de costo (viaje + espera esperada + desviación del plan) pero optimizan objetivos distintos. Los dos criterios clásicos, minimizar espera de camión ("maximizar camiones") y minimizar espera de pala ("maximizar palas"), entran en conflicto: no se pueden minimizar ambas, y ese es el núcleo multi-objetivo del despacho.' : 'Policies share a cost basis (travel + expected wait + plan deviation) but optimise different objectives. The two classic criteria, minimise truck wait ("maximise trucks") and minimise shovel wait ("maximise shovels"), conflict: both cannot be minimised at once, and that is the multi-objective core of dispatch.'}</p>
...
function Or({ es }: { es: boolean }) {
  return (<>
    ...
    <p>{es
      ? <>donde <InlineMath tex={String.raw`\mathrm{free}_s`} /> incluye el backlog comprometido (cola + camiones ya despachados en tránsito), omitir el en-tránsito re-crea el amontonamiento que la asignación conjunta existe para evitar (hallazgo de test). Resultado honesto del corpus: sobre el corpus round-2 más rico (ruteo multi-destino, rehandle a acopio y flotas heterogéneas) la asignación conjunta Hungarian ahora lidera el agregado (rango medio ~2.0 en los 8 casos), por delante de las heurísticas miopes, la vista de flota carga estructura cruzada real que la asignación instantánea aprovecha; en el corpus previo homogéneo de destino único no lo hacía, consistente con la literatura. El ranking vive en el Benchmark (leído del artefacto, no fijado en código).</>
      : <>where <InlineMath tex={String.raw`\mathrm{free}_s`} /> includes the committed backlog (queue + already-dispatched in-transit trucks), omitting in-transit re-creates the herding the joint assignment exists to avoid (a test-caught finding). Honest corpus result: on the richer round-2 corpus (multi-destination routing, stockpile rehandle, heterogeneous fleets) the joint Hungarian assignment now leads the aggregate (mean rank ~2.0 over the 8 cases), ahead of the myopic heuristics, the fleet view carries real cross-truck structure that instantaneous assignment exploits; on the earlier single-destination homogeneous corpus it did not, consistent with the literature. The live ranking is in the Benchmark (read from the artifact, not hardcoded).</>}</p>
...
function Rollout({ es }: { es: boolean }) {
  return (<>
    ...
    <p>{es
      ? 'El rollout verdadero (K x horizonte pasos DES por decisión) es demasiado pesado para el navegador. Así que se ejecuta offline sobre el corpus y sus acciones elegidas se destilan en una MLP por-pala pequeña -> dl-rollout.onnx, que se ejecuta en vivo con onnxruntime-web como las redes RWR/BC. El panel «Rollout inspector» de la App ejecuta un rollout acotado bajo demanda (K y horizonte pequeños, nunca en autoplay) para mostrar los K futuros por candidato.'
