  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">{es
          ? 'DispatchLab es un entorno para comparar políticas de despacho camión-pala sobre una simulación de eventos discretos determinista de un rajo, desde heurísticas clásicas hasta políticas aprendidas (redes entrenadas offline, inferencia ONNX en vivo).'
      ...
      <section>
        ...
        <p>{es
          ? 'En un rajo de camiones y palas, cada vez que un camión termina de descargar debe asignarse a su próxima pala. Esa decisión recurrente determina las toneladas por turno, la cola de camiones, la ociosidad de las palas (el recurso caro) y el cumplimiento de mezcla. Y no ocurre en el vacío: los camiones recorren una red de caminos real (portal, tronco compartido, ramales a chancador/botadero/acopio) donde el tiempo de viaje emerge del tráfico, del límite de velocidad, del seguimiento con distancia de seguridad (bunching, sin adelantar) y de la demora al cruzarse en doble vía. Es el problema de investigación de operaciones de la minería a cielo abierto: Codelco, BHP y Rio Tinto ejecutan sistemas comerciales (Modular DISPATCH, Wenco, MineStar) sobre exactamente este lazo.'
          : 'In a truck-shovel open pit, each time a truck finishes dumping it must be assigned to its next shovel. That recurring decision sets the tonnes per shift, the truck queue, the shovel idle time (the expensive resource) and blend compliance. And it does not happen in a vacuum: trucks travel a real road network (portal, shared trunk, spurs to crusher/waste/stockpile) where travel time emerges from traffic, a speed limit, car-following with a security distance (bunching, no overtaking) and a meeting slowdown on two-way sections. It is the operations-research problem of open-pit mining: Codelco, BHP and Rio Tinto run commercial systems (Modular DISPATCH, Wenco, MineStar) on exactly this loop.'}</p>
        <Refs ids={['white1986', 'alarie2002', 'soofastaei2016']} label="Refs" />

        <h2>{es ? 'El enfoque, clásico + aprendido' : 'The approach, classical + learned'}</h2>
        <ArchSVG es={es} />
        <p>{es
          ? 'Sobre un motor de eventos discretos determinista se ejecutan las políticas: las clásicas (fija, greedy, espera-mínima, los dos criterios en conflicto, asignación Hungarian) y las aprendidas, dos redes de puntuación por pala entrenadas offline sobre las decisiones registradas del propio simulador (imitación ponderada por recompensa + clon de la mejor heurística), exportadas a ONNX y ejecutadas en vivo en el navegador. El entorno mide honestamente las toneladas que cada una mueve, valida el simulador contra el match factor cerrado, y muestra el reparto de decisiones, las colas y el ciclo.'
        ...
        <p>{es ? 'Ingenieros de planificación y despacho, estudiantes y educadores de minería, investigadores de OR/RL. Es un sandbox didáctico y transparente de comparación de políticas, no un sistema de despacho productivo ni rastreo en planta.' : 'Mine planning + dispatch engineers, mining students/educators, OR/RL researchers. A didactic, transparent policy-comparison sandbox, not a production dispatch system nor in-plant tracking.'}</p>
